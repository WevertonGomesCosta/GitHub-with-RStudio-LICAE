...
Adicione um nome criativo e uma descrição ao seu repositório, deixe como opção pública, que permitirá que você crie uma página no GitHub para compartilhar seu trabalho com todos nós.
...
Então vamos para o RStudio.

Para clonar (Baixar) seu repositório do GitHub, clique no botão `Project` no canto superior direito do RStudio e depois em `New Project...`.
...
Isso abrirá uma janela com o `Clone Git Repository`. Aqui você vai apenas colar aquele link que eu pedi para você copiar há muito tempo na janela `Repository URL:`. Em `Criar projeto como subdiretório de:` selecione um diretório de fácil acesso para você, que facilitará sua localização posteriormente.
...
<br>
Então vamos criar a estrutura de diretório `workflowr` com a função `wflow_start`. *você já tem um diretório principal para o seu projeto, basta copiá-lo e colar aqui.*
...
```
Este repositório foi criado para auxiliar minha experiência de aprendizado com GitHub e workflowr.

Meu primeiro código R neste projeto será nesta [página do hub do git](PCA.html)
```

<br>

Isso é ótimo, mas ainda não temos o arquivo `PCA.hmtl`, então vamos criá-lo com a função `wflow_open`.

```{r Cria um novo arquivo Rmd, eval = FALSE}
wflow_open("análise/PCA.Rmd")
```

Isso deve criar o arquivo PCA.Rmd, você deve estar procurando por ele agora.
![](images/PCARmd.png){width="100%"}

<br>

Você pode atualizar o nome para substituir a abreviação de `Análise de Componentes Principais` e adicionar uma nova introdução para a análise que faremos neste arquivo R markdown.

Você pode seguir o exemplo deste site [WorkFlowRExample](https://lucianorogerio.github.io/WorkFlowRExample/).
...
wflow_publish(files = "analise/*.Rmd", mensagem = "Test")
```
...
Para confirmar suas atualizações, basta clicar no botão confirmar neste menu. Isso abrirá uma janela chamada `Rstudio: Review Changes`.
...
>*NÃO SE ESQUEÇA DE ESCREVER UMA BREVE MENSAGEM SIGNIFICATIVA SOBRE AS NOVAS MUDANÇAS PARA ESTE COMMIT*.

Basta clicar em `commit` e, em seguida, enviar seu novo commit para o GitHub clicando na seta verde (`Push`).
...
No site do seu repositório, clique em `settings`
...
Agora, todos que têm acesso ao seu repositório podem ver o site do seu projeto apenas clicando no link fornecido na seção sobre.
